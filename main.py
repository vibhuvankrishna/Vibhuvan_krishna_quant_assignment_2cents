...
CONFIG = [
    {"symbol": "ETHUSDT", "timeframe": "1m", "alpha": "momentum", "candles_to_keep": 40, "qty": 0.1, "broker":"iibkr"},
    {"symbol": "SOLUSDT", "timeframe": "1m", "alpha": "mean_revert", "candles_to_keep": 60, "qty": 1, "broker":"binance"},
    {"symbol": "BTCUSDT", "timeframe": "1m", "alpha": "lstm", "candles_to_keep": 50, "qty": 0.001, "broker":"ibkr"},
    {"symbol": "BNBUSDT", "timeframe": "3m", "alpha": "randomforest", "candles_to_keep": 60, "qty": 0.1, "broker":"ibkr"},
    {"symbol": "BTCUSDT", "timeframe": "1m", "alpha": "rf_lstm", "candles_to_keep": 50, "qty": 0.001, "broker":"ibkr"},
]
...
ALPHAS = {
    ...
    "randomforest" :alphas.alpha_rf,
    "lstm": alphas.alpha_lstm,
    "rf_lstm" : alphas.alpha_pred_models
}
...
# ---------- Helper Functions ----------
def get_ibkr_klines(symbol, interval, limit=50):
    """
    Fetch IBKR historical bars and convert to Binance-like format.
    Automatically handles PAXOS/CRYPTO fallback and converts timestamps to IST.
    """
    try:
        # ---- Parse symbol ----
        symbol = symbol.upper()
        # Common quote currencies
        known_quotes = ['USDT', 'USD']
        quote = next((q for q in known_quotes if symbol.endswith(q)), None)
        if not quote:
            raise ValueError(f"Unsupported quote in symbol: {symbol}")
        base = symbol.replace(quote, '')

        # IBKR uses USD instead of USDT
        if quote == 'USDT':
            quote_ibkr = 'USD'
        else:
            quote_ibkr = quote
        ...
        # ---- Contract Fallbacks ----
        exchanges = ['PAXOS', 'CRYPTO']  # Try PAXOS first, then CRYPTO
        bars = None
        for ex in exchanges:
            contract = Crypto(base, ex, quote_ibkr)
            try:
                bar_size, duration = IBKR_TIMEFRAMES[interval]
                ...
                logging.debug(f"No data from {ex} for {symbol}: {e}")
                bars = None

        if not bars:
            logging.warning(f"No IBKR data returned for {symbol}")
            return pd.DataFrame(columns=[
                'open_time','open','high','low','close','volume','close_time','open_time_ist','close_time_ist'
            ])
        ...
        df['close_time'] = df['open_time'] + pd.to_timedelta(INTERVAL_SECONDS[interval], unit='s')

        # Convert to IST
        df['open_time_ist'] = df['open_time'] + timedelta(hours=5, minutes=30)
        df['close_time_ist'] = df['close_time'] + timedelta(hours=5, minutes=30)

        # Keep relevant columns
        df = df[['open_time','open','high','low','close','volume','close_time','open_time_ist','close_time_ist']]
        ...
        logging.error(f"Failed to fetch IBKR klines for {symbol}: {e}")
        return pd.DataFrame(columns=[
            'open_time','open','high','low','close','volume','close_time','open_time_ist','close_time_ist'
        ])
    ...
    while True:
        for cfg in CONFIG:
            if datetime.now() >= cfg["next_min"]:
                ...
                if cfg["broker"] == "ibkr":
                    df = get_ibkr_klines(symbol, timeframe, cfg["candles_to_keep"])
                    # fallback if df is empty
                    if df.empty:
                        logging.warning(f"{symbol} not available on IBKR, switching to Binance Testnet...")
                        df = get_klines(symbol, timeframe, cfg["candles_to_keep"])
